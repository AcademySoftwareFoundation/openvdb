...
/// @param offset_mode EXPERIMENTAL parameter that will be removed in the near
...
public:
    ...
    /// @param mode EXPERIMENTAL parameter that will be removed in the near
    ///             future. It should only be used by experts!
    ...
    /// @brief Returns the finest/smallest voxel size, in world units, i.e. the
    ///        voxel size of the final (highest-resolution) level set surface.
    float minVoxelSize() const {return mMinVoxelSize;}

    /// @brief Returns the coarsest/largest voxel size, in world units, used to
    ///        initiate the coarse-to-fine shrink wrap algorithm.
    float maxVoxelSize() const {return mMaxVoxelSize;}

    /// @brief Returns the half-width of the output narrow-band level set, in voxel units.
    ...
    : mPoly(poly), mHalfWidth(width)
{
    ...
    // A too-small dim relative to halfWidth drives the denominator above to zero
