...
///
///          The paper defines the blended level set as
///          @c C=max(A,B)+m^beta*gamma. Since OpenVDB level sets use a
///          negative-inside convention, the implementation instead computes
///          @c C=min(A,B)-m^beta*gamma.
///          In the API below, @c alpha, @c beta, and @c gamma
///          correspond to @c blendRadius, @c falloffSharpness, and
...
///
/// @param blendRadius Controls the blend radius.
///
/// @param falloffSharpness Controls how sharply the fillet influence falls off.
///
/// @param filletStrength Controls the strength of the fillet offset.
///
...
unionFillet(const GridT& lhs,
     ...
     typename GridT::ValueType falloffSharpness,
     typename GridT::ValueType filletStrength,
...
struct UnionWithFillet {
    ...
    UnionWithFillet(const GridT& lhsGrid,
                    ...
                    const ValueType& falloffSharpness,
                    const ValueType& filletStrength)
        : mLhsGrid(&lhsGrid)
        , mRhsGrid(&rhsGrid)
        , mLhsTree(&(lhsGrid.tree()))
        , mRhsTree(&(rhsGrid.tree()))
        , mBlendRadius(blendRadius)
        , mFalloffSharpness(falloffSharpness)
        , mFilletStrength(filletStrength)
    {
...
private:
    struct FilletParms {
        float mBlendRadius;
        float mFalloffSharpness;
        float mFilletStrength;
    };
    ...
    ValueType mBlendRadius;
    ValueType mFalloffSharpness;
    ValueType mFilletStrength;
};
                       ...
                       int supportDilation)
{
    ...
    const typename GridT::ValueType supportRadius =
        blendRadius + typename GridT::ValueType(supportDilation * lhs.voxelSize()[0]);
        ...
        void operator()(const tbb::blocked_range<size_t>& range)
        {
            ...
            for (size_t n = range.begin(), N = range.end(); n < N; ++n) {
                ...
                if (rhsNodePt) { // combine overlapping nodes
                    ...
                    const float blendRadius = mParms.mBlendRadius;
                    const float falloffSharpness = mParms.mFalloffSharpness;
                    const float filletStrength = mParms.mFilletStrength;

                    for (openvdb::Index pos = 0; pos < LeafNodeType::SIZE; ++pos) {
                        const float A = lhsData[pos];
                        const float B = rhsData[pos];
                        const float m =
                            openvdb::math::Clamp((blendRadius - A) / blendRadius, 0.f, 1.f) *
                            openvdb::math::Clamp((blendRadius - B) / blendRadius, 0.f, 1.f);
                        ...
                        const float offset = hasValidFilletSamples ?
                            openvdb::math::Pow(m, falloffSharpness) * filletStrength : 0.0f;
...
typename GridT::Ptr UnionWithFillet<GridT, MaskT>::blend() {
    ...
    parms.mFilletStrength = mFilletStrength;
