...
{
    if (threaded) {
        mt::parallel_for(mt::blocked_range<size_t>(0, mLeafNodes.size()), *this);
    } else {
        (*this)(mt::blocked_range<size_t>(0, mLeafNodes.size()));
    }
}

template<typename Index32LeafT>
void
LeafOp<Index32LeafT>::operator()(const mt::blocked_range<size_t>& range) const
{
...
public:
    ...
    inline void operator()(const mt::blocked_range<size_t>&) const;
...
{
    if (threaded) {
        mt::parallel_for(mt::blocked_range<size_t>(0, mLeafRanges.size()), *this);
    } else {
        (*this)(mt::blocked_range<size_t>(0, mLeafRanges.size()));
    }
}

inline void
NodeOp::operator()(const mt::blocked_range<size_t>& range) const
{
    Vec3d avg, pos;
...
{
    if (threaded) {
        mt::parallel_for(mt::blocked_range<size_t>(0, mInstancePoints.size()), *this);
    } else {
        (*this)(mt::blocked_range<size_t>(0, mInstancePoints.size()));
    }
}
...
template<typename Index32LeafT>
void
ClosestPointDist<Index32LeafT>::operator()(const mt::blocked_range<size_t>& range) const
{
    Vec3R center;
    for (size_t n = range.begin(); n != range.end(); ++n) {
...
public:
    ...
    UpdatePoints(UpdatePoints&, mt::split);
...
inline
UpdatePoints::UpdatePoints(UpdatePoints& rhs, mt::split)
    : mSphere(rhs.mSphere)
    , mPoints(rhs.mPoints)
...
{
    if (threaded) {
        mt::parallel_reduce(mt::blocked_range<size_t>(0, mPoints.size()), *this);
    } else {
...
inline void
UpdatePoints::operator()(const mt::blocked_range<size_t>& range)
{
    Vec3s pos;
    for (size_t n = range.begin(); n != range.end(); ++n) {
        if (mMask[n]) continue;
    ...
    const GridT& grid, float isovalue, InterrupterT* interrupter)
{
    ...
    { // Extract surface point cloud
        ...
        mt::parallel_for(auxiliaryLeafNodeRange, volume_to_mesh_internal::ComputePoints<TreeT>(
