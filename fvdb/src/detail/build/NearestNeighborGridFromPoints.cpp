...
nanovdb::GridHandle<PytorchDeviceBuffer> buildNearestNeighborGridFromPointsCPU(const JaggedTensor& jaggedPoints,
    ...
    return AT_DISPATCH_FLOATING_TYPES_AND_HALF(jaggedPoints.scalar_type(), "buildNearestNeighborGridFromPoints", [&]() {
        ...
        const torch::TensorAccessor<int64_t, 2>& pointsBOffsetsAcc = jaggedPoints.joffsets().accessor<int64_t, 2>();

        std::vector<nanovdb::GridHandle<PytorchDeviceBuffer>> batchHandles;
        batchHandles.reserve(pointsBOffsetsAcc.size(0));
        for (int bi = 0; bi < pointsBOffsetsAcc.size(0); bi += 1) {
            ...
            const int64_t start = pointsBOffsetsAcc[bi][0];
