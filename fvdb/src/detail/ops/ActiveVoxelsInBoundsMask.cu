...
enabledGridVoxelInBoundsMaskCallback(int32_t batchIdx,
                                     int32_t leafIdx,
                                     int32_t voxelIdx,
...
void
GetEnabledVoxelsInBoundsMask(const GridBatchImpl &gridBatch,
...
ActiveVoxelsInBoundsMask(const GridBatchImpl &batchHdl,
    ...
    FVDB_DISPATCH_GRID_TYPES(batchHdl, [&]() {
        if (ignoreDisabledVoxels ||
            ...
            GetActiveVoxelsInBoundsMask<DeviceTag, GridType>(
                batchHdl, batchBboxes, outGridBoundsMask);
...
dispatchActiveVoxelsInBoundsMask<torch::kCUDA>(const GridBatchImpl &batchHdl,
                                               ...
                                               bool ignoreDisabledVoxels) {
    return ActiveVoxelsInBoundsMask<torch::kCUDA>(
        batchHdl, boundsMinIjk, boundsMaxIjk, ignoreDisabledVoxels);
}
...
dispatchActiveVoxelsInBoundsMask<torch::kCPU>(const GridBatchImpl &batchHdl,
                                              ...
                                              const Vec3iBatch &boundsMaxIjk,
                                              bool ignoreDisabledVoxels) {
    return ActiveVoxelsInBoundsMask<torch::kCPU>(
        batchHdl, boundsMinIjk, boundsMaxIjk, ignoreDisabledVoxels);
}
