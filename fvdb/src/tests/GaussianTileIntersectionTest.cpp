...
class GaussianTileIntersectionTest : public ::testing::Test {
  protected:
    ...
    void
    verifyTileDepthOrder(int start, int end, const torch::Tensor &intersection_values,
                         const torch::Tensor &depths) {
    ...
    verifyTileIntersections(const torch::Tensor &tile_offsets,
                            const torch::Tensor &intersection_values, const torch::Tensor &means2d,
                            const torch::Tensor &radii, const torch::Tensor &depths,
...
TEST_F(GaussianTileIntersectionTest, DenseViaSparseTest) {
    ...
    auto [tile_offsets, intersection_values] =
        fvdb::detail::ops::dispatchGaussianTileIntersectionSparse<torch::kCUDA>(
            means2d.cuda(), radii.cuda(), depths.cuda(), tile_mask.cuda(), active_tiles.cuda(),
            /*camera_jidx=*/at::nullopt, num_cameras, tile_size, num_tiles_h, num_tiles_w);
...
TEST_F(GaussianTileIntersectionTest, SparseCPUNotImplementedTest) {
    ...
    EXPECT_THROW(fvdb::detail::ops::dispatchGaussianTileIntersectionSparse<torch::kCPU>(
                     means2d, radii, depths, tile_mask, active_tiles,
                     /*camera_jidx=*/at::nullopt, num_cameras, tile_size, num_tiles_h, num_tiles_w),
                 c10::Error);
}
...
TEST_F(GaussianTileIntersectionTest, SparsePackedFormatTest) {
    ...
    auto [tile_offsets, intersection_values] =
        fvdb::detail::ops::dispatchGaussianTileIntersectionSparse<torch::kCUDA>(
            means2d_packed.cuda(), radii_packed.cuda(), depths_packed.cuda(), tile_mask.cuda(),
            active_tiles.cuda(), camera_indices.cuda(), num_cameras, tile_size, num_tiles_h,
            num_tiles_w);
...
TEST_F(GaussianTileIntersectionTest, RandomSparsePatternTest) {
    ...
    verifyTileIntersections(tile_offsets, intersection_values, means2d, radii, depths,
                            active_tiles);
