...
void Tool::init()
{
  ...
  mParser.addAction(
    ...
    {{"vdb", "0", "0", "age (i.e. stack index) of the VDB grid to be processed. Defaults to 0, i.e. most recently inserted VDB."},
     {"keep", "", "1|0|true|false", "toggle wether the input VDB is preserved or deleted after the processing"},
     {"file", "slice.ppm", "(0,0,0),(1,1,1)", "min and max of the world-space bounding-box used for clipping. Defaults to empty, i.e. disabled"},
     {"width", "512", "1", "taper of the frustum (requires bbox and depth to be specified). Defaults to -1, i.e. disabled"},
     {"height", "512", "1", "taper of the frustum (requires bbox and depth to be specified). Defaults to -1, i.e. disabled"},
     {"X", "0.5", "1", "taper of the frustum (requires bbox and depth to be specified). Defaults to -1, i.e. disabled"},
     {"Y", "0.5", "1", "taper of the frustum (requires bbox and depth to be specified). Defaults to -1, i.e. disabled"},
     {"Z", "0.5", "1", "taper of the frustum (requires bbox and depth to be specified). Defaults to -1, i.e. disabled"}},
...
void Tool::slice()
{
  ...
  try {
    ...
    const int width = mParser.get<int>("width");
    const int height = mParser.get<int>("height");
    const int X = mParser.get<float>("X");
    const int Y = mParser.get<float>("Y");
    const int Z = mParser.get<float>("Z");
    ...
    tools::Film image(width, height);
    for (int i=0; i<width; ++i) {
      const uint8_t n = uint8_t((255.0f*i)/(width-1));
      for (int j=0; j<height; ++j) {
        image.pixel(i,j) = tools::Film::RGBA(s*LUT[n][0], s*LUT[n][1], s*LUT[n][2]);
      }
    }
    image.savePPM(file);
