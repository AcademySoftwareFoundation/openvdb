# Actions (prefixed with `-`) and their options (`name=value`, except for file lists)
```
vdb_tool -sphere
...
# Internal stacks of VDB grids and geometry, addressed by age
```
...
# The `keep` option (preserve the input on the stack)
```
...
# Unix pipes: streaming VDB grids in and out of vdb_tool
```
...
# Enright benchmark test (analytic divergence-free advection)
```
