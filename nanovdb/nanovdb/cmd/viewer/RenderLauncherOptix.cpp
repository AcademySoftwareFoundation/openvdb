    ...
    if (it != mResources.end() && it->second->mMaterialClass == renderMethod) {
        ...
        if (NodeLevel >= 3) {
            ...
            for (int i = 0; i < numNodes; ++i) {
                ...
                auto* node = static_cast<typename nanovdb::NanoNode<float, NodeLevel>::Type*>(nodeAddress);
                //auto* node = static_cast<typename nanovdb::NodeTrait<nanovdb::FloatTree, NodeLevel>::type*>(nodeAddress);
