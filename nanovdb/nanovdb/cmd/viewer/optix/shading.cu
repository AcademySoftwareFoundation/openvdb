...
extern "C" __global__ void __closesthit__nanovdb_fogvolume_radiance()
{
    ...
    medium.densityMin = render::valueToScalar(densityGrid->tree().root().minimum()) * medium.densityScale;
    medium.densityMax = render::valueToScalar(densityGrid->tree().root().maximum()) * medium.densityScale;
...
extern "C" __global__ void __closesthit__nanovdb_fogvolume_occlusion()
{
    ...
    HeterogenousMedium medium;
    medium.densityScale = params.volumeDensityScale;
    medium.densityMin = densityGrid->tree().root().minimum() * medium.densityScale;
    medium.densityMax = densityGrid->tree().root().maximum() * medium.densityScale;
    medium.densityMax = fmaxf(medium.densityMin, fmaxf(medium.densityMax, 0.001f));
