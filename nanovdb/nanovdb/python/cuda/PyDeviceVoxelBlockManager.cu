...
{
    VoxelBlockManagerHandle<nanovdb::cuda::DualDeviceBuffer> handle;
    int                                                  log2BlockWidth = 6;

    PyDeviceVBMHandle() = default;
    PyDeviceVBMHandle(VoxelBlockManagerHandle<nanovdb::cuda::DualDeviceBuffer>&& h,
                      int lbw) noexcept
...
// with per-node statistics / tile values (the createNanoGridOnIndex defaults) has
                                          ...
                                          const char* fn_name)
{
    ...
    if (valueCount != activeCount + 1) {
        ...
        msg += ": requires an OnIndex grid with contiguous active-voxel indexing "
               ...
               "createNanoGridOnIndex(grid, includeStats=False, includeTiles=False) "
            ...
            return dispatchLog2BlockWidth(log2BlockWidth, [&](auto W) {
                ...
                VoxelBlockManagerHandle<nanovdb::cuda::DualDeviceBuffer> handle;
                {
                    ...
                    handle = nanovdb::tools::cuda::buildVoxelBlockManager<
                        LBW, nanovdb::cuda::DualDeviceBuffer>(
           ...
           int log2BlockWidth, uintptr_t stream) {
            ...
            dispatchLog2BlockWidth(log2BlockWidth, [&](auto W) {
                ...
                auto handle = nanovdb::tools::cuda::buildVoxelBlockManager<
                    LBW, nanovdb::cuda::DualDeviceBuffer>(dGrid, 0, 0, 0, s);
