...
| [`cupy_gather_stencil.py`](cupy_gather_stencil.py) | The **kernel-free** counterpart: `tools.cuda.gatherBoxStencil` materialises each active voxel's 3×3×3 neighbourhood into a dense `(valueCount, 27)` array, then the Laplacian deform and Godunov reinitialisation run as plain **CuPy** array math — no `cupy.RawModule`, no CUDA kernel, no coordinates (the same dense array a cuTile `@ct.kernel` would `ct.load`). Verified by an invariant: reinitialisation drives \|∇φ\|→1 after a deform degrades it. Requires only CuPy (no nvcc/headers). |
