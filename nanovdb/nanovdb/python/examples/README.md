...
| [`build_grid.py`](build_grid.py) | Voxel-by-voxel construction with `nanovdb.tools.build.FloatGrid`, the cached `ValueAccessor`, the thread-safe `WriteAccessor`, and `.toNanoVDB()` to bake into a host `GridHandle`. |
| [`bulk_leaf_numpy.py`](bulk_leaf_numpy.py) | Zero-copy `(N_leaves, 512)` NumPy view of every leaf's mValues via `grid.leafValues()`. Includes a global-stats reduction and an in-place mutation that propagates back into the grid. Requires NumPy. |
...
| [`tree_stats.py`](tree_stats.py) | Real top-down tree walk via `Upper.getChild(n)` / `Lower.getChild(n)` and `isChild(n)` — pretty-prints per-upper lower-child counts and per-lower leaf-child counts, unlike `node_manager.py`'s flat per-level lists. |
...
onh = nanovdb.tools.createNanoGridOnIndex(fg)    # host OnIndexGrid handle
