...
def _build_device_onindex_grid(radius=20.0):
    ...
    fg = h.grid(0)
    onh = nanovdb.tools.createNanoGridOnIndex(fg)
...
@unittest.skipIf(
    not hasattr(getattr(nanovdb, "cuda", None), "compile_options"),
    "compile_options is attached by the wheel's nanovdb/__init__.py; "
    "not available when importing the in-tree extension directly",
)
...
class TestDeviceNodeManager(unittest.TestCase):
    ...
    def test_returns_typed_device_node_manager(self):
        dh, dg = _build_device_onindex_grid(20.0)
        mgr = nanovdb.cuda.createDeviceNodeManager(dg, 0)
        # The TYPED device NodeManager comes back directly (no handle hop),
        # matching the host nanovdb.createNodeManager.
        self.assertIsInstance(mgr, nanovdb.OnIndexNodeManager)
        self.assertFalse(hasattr(mgr, "mgr"))

    def test_manager_outlives_temporary_grid(self):
        import gc
        mgr = nanovdb.cuda.createDeviceNodeManager(
            _build_device_onindex_grid(20.0)[1], 0)
        gc.collect()
        # keep_alive parents the manager to the device grid, which in turn
        # holds its DeviceGridHandle; the manager stays valid after the
        # temporaries went out of scope.
        self.assertIsInstance(mgr, nanovdb.OnIndexNodeManager)
...
class TestDeviceVoxelBlockManager(unittest.TestCase):
    ...
    def test_gather_rejects_noncontiguous_grid(self):
        ...
        NOT contiguous -- e.g. createNanoGridOnIndex's default includeStats /
        includeTiles, which add value slots past activeVoxelCount."""
        cp = _require_cupy(self)
        dh, dg = _build_device_onindex_grid(20.0)   # createNanoGridOnIndex defaults: stats+tiles
