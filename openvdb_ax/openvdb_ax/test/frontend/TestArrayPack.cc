...
TEST_F(TestArrayPack, testASTNode)
{
    for (const auto& test : tests) {
        ...
        ASSERT_TRUE(result);
        ASSERT_TRUE(Node::ArrayPackNode == result->nodetype()) << ERROR_MSG("Invalid AST node", code);
        ...
        if (!unittest_util::compareLinearTrees(expectedList, resultList)) {
            ...
            FAIL() << ERROR_MSG("Mismatching Trees for Array Pack code", code) + os.str();
        }
    }
}
