...
class TestTernaryOperatorNode : public ::testing::Test
{
};

TEST_F(TestTernaryOperatorNode, testSyntax)
{
    TEST_SYNTAX_PASSES(tests);
}

TEST_F(TestTernaryOperatorNode, testASTNode)
{
    for (const auto& test : tests) {
        ...
        ASSERT_TRUE(result);
        ASSERT_TRUE(Node::TernaryOperatorNode == result->nodetype()) << ERROR_MSG("Invalid AST node", code);
        ...
        if (!unittest_util::compareLinearTrees(expectedList, resultList)) {
            ...
            FAIL() << ERROR_MSG("Mismatching Trees for Ternary Operator code", code) + os.str();
        }
    }
}
