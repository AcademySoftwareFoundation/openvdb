...
class TestDeclareLocalNode : public ::testing::Test
{
};

TEST_F(TestDeclareLocalNode, testSyntax)
{
    TEST_SYNTAX_PASSES(tests);
}

TEST_F(TestDeclareLocalNode, testASTNode)
{
    for (const auto& test : tests) {
        ...
        ASSERT_TRUE(result);
        ASSERT_TRUE(Node::DeclareLocalNode == result->nodetype()) << ERROR_MSG("Invalid AST node", code);
        ...
        if (!unittest_util::compareLinearTrees(expectedList, resultList)) {
            ...
            FAIL() << ERROR_MSG("Mismatching Trees for Declaration code", code) + os.str();
        }
    }
}
