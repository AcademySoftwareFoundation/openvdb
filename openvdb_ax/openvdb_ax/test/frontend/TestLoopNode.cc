...
class TestLoopNode : public ::testing::Test
{
};

TEST_F(TestLoopNode, testSyntax)
{
    TEST_SYNTAX_PASSES(tests);
}

TEST_F(TestLoopNode, testASTNode)
{
    for (const auto& test : tests) {
        ...
        ASSERT_TRUE(result);
        ASSERT_TRUE(Node::LoopNode == result->nodetype()) << ERROR_MSG("Invalid AST node", code);
        ...
        if (!unittest_util::compareLinearTrees(expectedList, resultList)) {
            ...
            FAIL() << ERROR_MSG("Mismatching Trees for Loop code", code) + os.str();
        }
    }
}
